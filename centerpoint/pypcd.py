    ...
    *TODO* allow 'proper' multi-count fields.
    ...
    for f, c, t, s in zip(metadata['fields'],
        ...
        if c == 1:
            ...
            fieldnames.extend(['%s%s_%04d' % (multi_count_fields, f, i) for i in range(c)])
            typenames.extend([np_type]*c)
            multi_count_fields  = multi_count_fields + 1
...
def point_cloud_to_fileobj(pc, fileobj, data_compression=None):
    ...
    if metadata['data'].lower() == 'ascii':
        ...
        if buf is None:
            ...
            # Answere for above - write as Binary
...
def save_xyz_label(pc, fname, use_default_lbl=False):
    ...
    with open(fname, 'w') as f:
        for i in range(pc.points):
            ...
            lbl = '1000' if use_default_lbl else '%.4f' % pc.pc_data['label'][i]
...
def make_xyz_intensity_point_cloud(xyzi):
    """ Make XYZL point cloud from numpy array.
    ...
    pc_data = np.rec.fromarrays([xyzi[:, 0], xyzi[:, 1], xyzi[:, 2],
                                 xyzi[:, 3]], dtype=dt)
    pc = PointCloud(md, pc_data)
    return pc


def make_xyz_intensity_rgb_point_cloud(xyzi_rgb, metadata=None):
    """ Make a pointcloud object from xyz, intensity, rgb array.
    xyz array is assumed to be float32.
    rgb is assumed to be encoded as float32 according to pcl conventions.
    """
    md = {'version': .7,
          ...
          'size': [4, 4, 4, 4, 4],
          'data': 'binary'}
    if xyzi_rgb.dtype != np.float32:
        raise ValueError('array must be float32')
    if metadata is not None:
        md.update(metadata)
    pc_data = xyzi_rgb.view(np.dtype([('x', np.float32),
                                     ('y', np.float32),
                                     ('z', np.float32),
                                      ('intensity', np.float32),
                                     ('rgb', np.float32)])).squeeze()
